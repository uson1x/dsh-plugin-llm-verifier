/**
 * Browser half of dsh-plugin-llm-verifier: a rich chat card for the
 * `verify_rollout` tool, registered into the keyed `tool.call.toolview` slot.
 *
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** One rollout line: label, reward bar, score or failure reason. */
    function rolloutRow(index, meta) {
      ...
      if (reward === null || reward === undefined) {
        return h('div', { key: index, style: { ...styles.row, ...styles.failed } },
          ...
          h('span', null, `failed${stop && stop.stop_reason ? ` (${String(stop.stop_reason).slice(0, 80)})` : ''}`))
      }
    ...
    /** Client plugin body: claim the verify_rollout tool row. */
    function apply(ctx) {
