 ...
 *  - an optional per-session "Verifier" tab next to Chat/Trajectory
 *    (`conversation.view` slot ring), hidable via localStorage.
 *
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** localStorage key that hides the Verifier tab (per browser). */
    const HIDE_TAB_KEY = 'dsh-llm-verifier:tab'

    function tabHidden() {
      try { return window.localStorage.getItem(HIDE_TAB_KEY) === 'hidden' } catch { return false }
    }
    ...
    const styles = {
      ...
      footer: { opacity: 0.55, fontSize: 12, display: 'flex', flexDirection: 'column', gap: 4 },
    }
    ...
    function VerifierTab(props) {
      ...
      return h('div', { style: styles.tab },
        h('div', { style: styles.tabInner },
          body,
          h('div', { style: styles.footer },
            h('button', { style: { ...styles.toggle, opacity: 0.55 }, onClick: props.hideTab }, 'hide this tab'),
            h('span', null, `hiding is per-browser; re-enable in the console: localStorage.removeItem('${HIDE_TAB_KEY}') then reload`))))
    }
    ...
    function apply(ctx) {
      ...
      if (tabHidden()) return
      const tab = { dispose: null }
      ctx.slots.inject('conversation.view', () => {
        tab.dispose = ctx.slots.register({
          ...
          inject: (sessionId) => ({
            ...
            hideTab: () => {
              try { window.localStorage.setItem(HIDE_TAB_KEY, 'hidden') } catch { /* private mode */ }
