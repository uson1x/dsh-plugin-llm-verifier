...
export const Config = Schema.object({
  provider: Schema.string().required().description('Registered ctx.llm provider route the verifier grades with.'),
  model: Schema.string().required().description('Model id on that route.'),
  ...
  reasoningEffort: Schema.string().description('Optional adapter-owned reasoning effort for grading calls (e.g. a low effort keeps them fast and inside the token budget).'),
  maxOutputTokens: Schema.number().default(16384).description('Output cap per grading call (reasoning tokens count against it).'),
  timeoutMs: Schema.number().default(300000).description('Deadline per grading call in milliseconds; a call killed by this deadline is retried once.'),
  ...
  judgeTrace: Schema.union(['full', 'final']).default('full').description('What the verify_rollout judge sees per attempt: the full trajectory (tool calls and results included) or only the final message.'),
  groundTruthNote: Schema.union([Schema.string(), Schema.const(false)]).description('Note the judge always sees (default: "no reference solution available" per the reference implementation); false disables it.'),
  majorityVoting: Schema.boolean().default(true).description('Skip the tournament when a strict majority of candidates are identical.'),
  seed: Schema.number().description('Seed for the random ring pass, for reproducible tournaments.'),
  traceMaxChars: Schema.number().default(24000).description('Character budget per attempt trajectory shown to the judge; longer traces keep their head and tail.'),
  ...
  rollout: Schema.object({
    ...
    criteria: Schema.array(Schema.object({
      name: Schema.string().required(),
      description: Schema.string().required(),
    })).description('Judging criteria for full rollout trajectories (default: the reference implementation\'s single Task Success criterion).'),
...
/** Render one model-history message as compact trace lines for the judge. */
function traceLinesFrom(message) {
  const lines = []
  for (const block of message.content ?? []) {
    ...
    if (block.type === 'text' && typeof block.text === 'string' && block.text.trim().length > 0) {
      ...
      lines.push(`[tool call] ${block.name}(${args.length > 400 ? `${args.slice(0, 400)}…` : args})`)
    } else if (block.type === 'tool-result') {
      const text = textFromBlocks(block.content)
      lines.push(`[tool result]${block.isError ? ' (error)' : ''} ${text.length > 600 ? `${text.slice(0, 600)}…` : text}`)
    }
    // reasoning blocks stay private; image blocks carry no judgeable text
  }
  return lines
}

/**
 * Build the judge-facing trajectory for one rollout: every model-visible step
 * after the task prompt, bounded to `maxChars` (head and tail kept), with the
 * final deliverable always intact at the end.
 */
function formatTrace(messages, finalText, maxChars) {
  const lines = []
  let sawUser = false
  for (const message of messages ?? []) {
    if (message.role === 'user' && !sawUser) { sawUser = true; continue } // the task prompt itself
    lines.push(...traceLinesFrom(message))
  }
  let trace = lines.join('\n')
  if (trace.length > maxChars) {
    const head = Math.floor(maxChars * 0.6)
    const tail = maxChars - head
    trace = `${trace.slice(0, head)}\n…[trace truncated]…\n${trace.slice(-tail)}`
  }
  if (trace.length === 0) return finalText
  return `Trajectory of the attempt:\n${trace}\n\nFinal deliverable:\n${finalText}`
}
...
/** Count model-visible activity in a rollout's derived messages. */
function traceStats(messages) {
  let toolCalls = 0
  let assistantTurns = 0
  for (const message of messages) {
    if (message.role === 'assistant') assistantTurns += 1
    for (const block of message.content ?? []) {
      if (block !== null && typeof block === 'object' && block.type === 'tool-call') toolCalls += 1
    }
  }
  return { toolCalls, assistantTurns }
}

/** Run one rollout child to completion, always disposing the run. */
async function runRollout(subagents, providerName, request, trace) {
  const startedAt = Date.now()
  const run = await subagents.start(providerName, request)
  try {
    const result = await run.result
    const text = textFromBlocks(result.output)
    let judgeText = text
    let stats = null
    try {
      const messages = run.localAgent?.session?.deriveMessages?.()
      if (Array.isArray(messages)) {
        stats = traceStats(messages)
        if (trace.mode === 'full') judgeText = formatTrace(messages, text, trace.maxChars)
      }
    } catch {
      // A trace is an enrichment; judging falls back to the final message.
    }
    return {
...
export function apply(ctx, config = {}) {
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      // Replayable scoreboard for the web card (client half of this plugin).
      // Pure projection of the canonical value; no I/O, no clock.
      presentationMeta: (_args, value) => ({
        best_index: value.best_index,
        best_reward: value.best_reward,
        rewards: value.rewards,
        sessions: value.rollout_sessions,
        judge_trace: value.details?.judge_trace ?? null,
        stop_reasons: (value.details?.stop_reasons ?? []).map(r => ({ rollout: r.rollout, ok: r.ok, stop_reason: r.stop_reason })),
        winner_preview: value.winner.length > 20_000 ? `${value.winner.slice(0, 20_000)}…[truncated]` : value.winner,
        attempts: value.details?.attempts ?? null,
        judge: value.details?.judge ?? null,
      }),
    },
    async execute(args, exec) {
      const n = args.n ?? 3
      ...
      const rollouts = await mapLimit(Array.from({ length: n }, (_, i) => i), rolloutConfig.maxConcurrent, async index => {
        try {
          ...
          }, { mode: engine.config.judgeTrace, maxChars: engine.config.traceMaxChars })
          const ok = outcome.stopReason === 'completed' && outcome.text.length > 0
          return { index, ok, ...outcome }
        } catch (error) {
          if (exec.signal.aborted) throw error
          return { index, ok: false, sessionId: null, stopReason: `start-failed: ${String(error?.message ?? error)}`, text: '', durationMs: null, stats: null, judgeChars: 0 }
        }
      })
      ...
      const judged = await engine.select(args.task, succeeded.map(r => r.judgeText), { signal: exec.signal, criteria: engine.config.rollout.criteria })
      ...
      return {
        ...
        details: {
          judge_trace: engine.config.judgeTrace,
          stop_reasons: rollouts.map(r => ({ rollout: r.index + 1, ok: r.ok, stop_reason: r.stopReason, session: r.sessionId })),
          attempts: rollouts.map(r => ({
            rollout: r.index + 1,
            ok: r.ok,
            stop_reason: r.stopReason,
            session: r.sessionId,
            reward: rewards[r.index],
            duration_ms: r.durationMs ?? null,
            tool_calls: r.stats?.toolCalls ?? null,
            assistant_turns: r.stats?.assistantTurns ?? null,
            judge_chars: r.judgeChars ?? null,
            // Full deliverable (UI scrolls it); 20k-char ceiling only to keep
            // one pathological rollout from bloating the persisted event.
            preview: r.text.length > 20_000 ? `${r.text.slice(0, 20_000)}…[truncated]` : r.text,
          })),
          judge: {
            granularity: engine.config.granularity,
            repetitions: engine.config.repetitions,
            criteria: engine.config.rollout.criteria.map(c => c.name),
            // PPT internals, mapped from judged (successful-only) indices back
            // to 0-based original rollout indices.
            ring_scores: judged.ringScores.map((score, judgedIndex) => ({ rollout: succeeded[judgedIndex].index, score })),
            pivots: judged.pivots.map(judgedIndex => succeeded[judgedIndex].index),
            pairs: judged.pairs.map(p => ({ a: succeeded[p.i].index, b: succeeded[p.j].index, reward_a: p.rewardI, reward_b: p.rewardJ })),
          },
        },
      }
    },
